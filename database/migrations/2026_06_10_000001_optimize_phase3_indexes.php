 ...
 *
 *  3. invoices       — Missing composite [tenant_id, due_date] index.
 *                      The SendInvoiceReminders artisan command filters by
 *                      tenant_id + due_date across the full invoices table.
 ...
 *                      The composite [tenant_id, related_type, related_id] index
...
return new class extends Migration
{
    ...
    public function up(): void
    {
        ...
        // 3. invoices — composite index on [tenant_id, due_date]
        // The SendInvoiceReminders artisan command runs nightly and filters:
        //   WHERE status = 'sent' AND due_date < today()
        // On the invoices table (already indexed by [tenant_id, deleted_at, status]),
        ...
        // 4. communications — composite index on [tenant_id, related_type, related_id]
        ...
        //   WHERE tenant_id = ? AND related_type = 'App\Models\Invoice' AND related_id = ?
    ...
    {
        Schema::table('communications', function (Blueprint $table) {
            $table->dropIndex('communications_tenant_related_index');
        });
